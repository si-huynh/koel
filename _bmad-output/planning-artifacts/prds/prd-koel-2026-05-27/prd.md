...
- **OQ-Conformance-Equivalence — RESOLVED.** The `AgUiEvent_equal` rule for SC-1 is final: `freezed`-generated `==` covers every field, with `Uint8List` compared **byte-equal** (the sole binary event field, `ReasoningEncryptedValueEvent.encryptedValue`, is correctly content-identity — no event type needs identity/normalized comparison), and **no id-normalization layer is required** — conformance is corpus-graded against the synthesized canonical-id corpus while real-backend fidelity is graded separately by byte-round-trip equality, so backend-specific ids match trivially. Recorded in [`koel_core/CONFORMANCE.md`](../../../../packages/koel_core/CONFORMANCE.md) §`OQ-Conformance-Equivalence` (Story 9.9). Owner: P1.

...
- **OQ-Koel-Trademark.** Trademark search for "koel" beyond pub.dev (USPTO, EU IPO, India IP) in the software classes (Nice 9 + 42) prior to v1.0.0 publish. *Search attempted + recorded in [`trademark-search-koel.md`](../../../legal/trademark-search-koel.md) (Story 9.8); still **OPEN** — the authoritative register portals are not agent-accessible (JS/CAPTCHA-gated) and a same-name FOSS project ("Koel" music server, unregistered, different domain) is a flagged naming consideration, so final clearance is the owner's call. Blocks v1.0.0 publish.* Owner: P1.

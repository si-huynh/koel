...
- **OQ-Koel-Trademark.** Trademark search for "koel" beyond pub.dev (USPTO, EU IPO, India IP) prior to v1.0.0 publish. *Resolver: Story 9.8 (→ `_bmad-output/legal/trademark-search-koel.md`); still open — blocks v1.0.0 publish.* Owner: P1.
- **OQ-AGUI-License.** License-compatibility verification of the community `ag_ui` 0.1.0 package before the inspired-by README credit ships. *Resolver: Story 9.8 (→ `_bmad-output/legal/ag_ui-license-verification.md`); still open — blocks first published README that credits it.* Owner: P1.

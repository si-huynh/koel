...
- **AC #8 / #9 — OWNER-GATED, HANDED OFF.** The irreversible `dart pub publish` / `git tag` / `gh release create` are not run as a side effect of implementation (pub.dev archives are permanent; requires the `sihuynh.dev` authenticated publisher the agent does not own). No in-session go-ahead was given → handed off via `RELEASING.md`, recorded owner-pending. **Go/no-go surfaced to Si.**

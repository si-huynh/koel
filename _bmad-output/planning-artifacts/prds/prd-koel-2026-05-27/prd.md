...
- **SC-3.** `dart analyze` zero warnings across every package, default lint set + `package:lints/strict.yaml` overrides + `package:koel_lints/koel.yaml` mandatory rules (see §11 / F-A12).
...
- **N-13. `dart analyze` Clean.** Zero warnings on default lint set plus `package:lints/strict.yaml` overrides. CI gate.
...
- **OQ-Docs-Framework.** Docusaurus vs Nextra vs alternative for the dedicated docs site. *Resolve before v0.x docs site deploys; does not block code work.* Owner: P1.
...
- **OQ-Koel-Trademark.** Trademark search for "koel" beyond pub.dev (USPTO, EU IPO, India IP) prior to v1.0.0 publish. *Blocks v1.0.0 publish.* Owner: P1.
- **OQ-AGUI-License.** License-compatibility verification of the community `ag_ui` 0.1.0 package before the inspired-by README credit ships. *Blocks first published README that credits it.* Owner: P1.
...
- **OQ-Perf-Baseline.** The reference device profile, benchmark workloads, and v1.0.0 baseline numbers for §10.1 N-1 through N-4. *Resolve before v1.0.0 publish — CI cannot enforce regression-relative SLOs without baseline numbers.* Owner: P1.
- **OQ-Conformance-Equivalence.** The precise structural-equality rule for `AgUiEvent_equal` in SC-1 — does `freezed`-generated `==` cover every field (incl. `Uint8List` byte-equal vs identity)? *Resolve before v1.0.0 publish — SC-1 is not CI-enforceable without it.* Owner: P1.
